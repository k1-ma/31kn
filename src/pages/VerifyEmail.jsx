...
export default function VerifyEmail() {
  ...
  useEffect(() => {
    if (!token) {
      setStatus("error");
      setErrorMessage(t("auth.verifyEmail.errors.noToken") || "No verification token provided");
      return;
    }
  ...
  const verifyToken = async () => {
    setStatus("loading");
    try {
      const result = await apiJson("/api/auth/verify-email", {
        method: "POST",
        body: { token },
      });
      ...
      if (errorCode === "TOKEN_EXPIRED") {
        setErrorMessage(t("auth.verifyEmail.errors.tokenExpired") || "Verification link has expired. Please request a new one.");
      } else if (errorCode === "TOKEN_INVALID") {
        setErrorMessage(t("auth.verifyEmail.errors.tokenInvalid") || "Invalid verification link.");
      } else {
        setErrorMessage(e?.message || t("auth.verifyEmail.errors.failed") || "Verification failed. Please try again.");
      }
    }
  };
        ...
        return (
          <>
            ...
            <CardTitle className="text-2xl font-bold text-center text-success">
              {t("auth.verifyEmail.success.title") || "Email Verified!"}
            </CardTitle>
            <p className="text-sm text-muted-foreground text-center mt-2">
              {t("auth.verifyEmail.success.message") || "Your email has been successfully verified. You can now log in to your account."}
            </p>
            <div className="mt-6">
              <Link to="/login">
                <Button className="w-full h-10 rounded-lg">
                  {t("auth.verifyEmail.goToLogin") || "Go to Login"}
                </Button>
              </Link>
            </div>
          </>
      ...
      case "already":
        return (
          <>
            ...
            <CardTitle className="text-2xl font-bold text-center">
              {t("auth.verifyEmail.already.title") || "Already Verified"}
            </CardTitle>
            <p className="text-sm text-muted-foreground text-center mt-2">
              {t("auth.verifyEmail.already.message") || "This email has already been verified. You can log in to your account."}
            </p>
            <div className="mt-6">
              <Link to="/login">
                <Button className="w-full h-10 rounded-lg">
        ...
        return (
          <>
            ...
            <CardTitle className="text-2xl font-bold text-center text-danger">
              {t("auth.verifyEmail.error.title") || "Verification Failed"}
            </CardTitle>
            <p className="text-sm text-muted-foreground text-center mt-2">
              {errorMessage}
            </p>
